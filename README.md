...
* I'm changing the triggers for when to end a packet capture. It used to be when the player status was `ended` but now it will look for when the video is fully loaded, even though the video itself is still playing. This speeds up crawling, and we're only interested in the network traffic anyway. If it can't get an initial player status within the first 30 seconds, that will also terminate the visit instead of waiting for the 20-minute hard timeout.

* I'm setting the `--snapshot-length` to 34 bytes for tcpdump, so we only save the Ethernet and IP headers. We just need the source and destination addresses in the IP headers to filter out non-Tor traffic at the end of each visit, and then we need the packet length for our analysis. This reduces required storage space by roughly 98%.
